...
- `settings.json`: default Gmail sender metadata, Google Sheets destination settings, optional Trip Costs spreadsheet id, and the local database path.
...
- Trip mode is selected with `--trip` or `--trip-sheet-name`. It skips categorization and appends parsed `Item` and `Cost` rows to the named tab in the Trip Costs spreadsheet.
